...
public class Main {
	...
	public static void main(final String[] args) {
		try {
			...
			final Graph syndrome = new GraphBuilder().copyGraph(graph).randomFaulty().getGraph();
			final Graph Lgraph = new GraphBuilder().copyGraph(syndrome).calculateLgraph().getGraph();
	...
	public static void testMgraph(Graph graph) {
		...
		for (int i = 0; i < 1000; i++) {
			final Graph graph2 = new GraphBuilder().copyGraph(graph).randomFaulty().calculateLgraph()
